...
{
    // 1. 配置IRQ: 启用前导码检测、RX_DONE和错误中断
    SetDioIrqParams(E22_IRQ_PREAMBLE_DETECT | E22_IRQ_HEADER_VALID | 
                    E22_IRQ_RX_DONE | E22_IRQ_CRC_ERROR | E22_IRQ_HEADER_ERROR,
...
void E22_PollTask(void)
{
    ...
    if(irqStatus != 0) {
        // 检测到前导码或头部，说明正在接收
        if(irqStatus & (E22_IRQ_PREAMBLE_DETECT | E22_IRQ_HEADER_VALID)) {
            // 通过extern变量通知应用层
            extern volatile bool App_IsReceiving;
            extern void App_OnReceivingStart(void);
            App_IsReceiving = true;
            App_OnReceivingStart();
        }
