...
static RepeatSelection_t repeatSelection = REPEAT_SEL_CONTENT;
static char repeatContent[INPUT_BUFFER_SIZE];
static uint8_t repeatContentLen = 0;
static uint8_t repeatInterval = 1; // 默认1秒
static uint32_t repeatLastSendTime = 0;
static bool repeatEditMode = false;
static char repeatEditBuffer[8];
static uint8_t repeatEditLen = 0;

/* ======================== Receiving State ======================== */

volatile bool App_IsReceiving = false;
...
static const char *MenuStrings[] = {"Send Message", "View History", "Settings",
                                    "Repeat Send"};

static const char *SettingNames[] = {"Freq(kHz)", "Power(dBm)", "SF", "BW",
                                     "CR"};
...
static void GetSettingValueString(SettingItem_t setting, char *buf) {
    ...
    switch (setting) {
    case SETTING_FREQ:
        sprintf(buf, "%lu", (unsigned long)params.frequency / 1000);
        break;
    case SETTING_POWER:
...
static void DrawMenu(void) {
    ...
    // 显示接收指示器R
    if (App_IsReceiving) {
        OLED_DrawStringInverse(122, 0, "R", 1);
    }
...
static void DrawHistoryScreen(void) {
    ...
    if (msgHistoryCount == 0) {
        ...
        // Show message info
        uint8_t idx = (msgHistoryHead + MSG_HISTORY_SIZE - msgHistoryCount +
                       historyViewIndex) %
                      MSG_HISTORY_SIZE;
        ...
        // Navigation hint
        OLED_DrawString(0, 24, "L/R:Nav BS:Back", 1);
    }
...
static void DrawRepeatConfigScreen(void) {
    OLED_Clear();

    OLED_DrawString(0, 0, "= Repeat Send =", 1);
    ...
    // 显示内容预览 (截取前18个字符)
    char contentPreview[20];
    memset(contentPreview, 0, sizeof(contentPreview));
    if (repeatContentLen > 0) {
        uint8_t previewLen = repeatContentLen > 18 ? 18 : repeatContentLen;
        memcpy(contentPreview, repeatContent, previewLen);
    } else {
        strcpy(contentPreview, "(empty)");
    }

    if (repeatSelection == REPEAT_SEL_CONTENT) {
        if (repeatEditMode) {
            // 编辑内容模式
            OLED_DrawString(0, 10, "Content:", 1);
            OLED_DrawStringInverse(50, 10, contentPreview, 1);
        } else {
            OLED_DrawStringInverse(0, 10, "Content:", 1);
            OLED_DrawString(50, 10, contentPreview, 1);
        }
    } else {
        OLED_DrawString(0, 10, "Content:", 1);
        OLED_DrawString(50, 10, contentPreview, 1);
    ...
    // 显示间隔
    char intervalStr[16];
    if (repeatEditMode && repeatSelection == REPEAT_SEL_INTERVAL) {
        snprintf(intervalStr, sizeof(intervalStr), "%ss", repeatEditBuffer);
    } else {
        snprintf(intervalStr, sizeof(intervalStr), "%ds", repeatInterval);
    }

    if (repeatSelection == REPEAT_SEL_INTERVAL) {
        OLED_DrawStringInverse(0, 20, "Interval:", 1);
        OLED_DrawString(56, 20, intervalStr, 1);
    } else {
        OLED_DrawString(0, 20, "Interval:", 1);
        OLED_DrawString(56, 20, intervalStr, 1);
    }

    OLED_DrawString(0, 28, "ENT:Start BS:Back", 1);

    OLED_Update();
}

static void DrawRepeatRunningScreen(void) {
    OLED_Clear();

    OLED_DrawString(0, 0, "= Repeating =", 1);
    ...
    uint8_t previewLen = repeatContentLen > 18 ? 18 : repeatContentLen;
    memcpy(contentPreview, repeatContent, previewLen);
    OLED_DrawString(0, 10, contentPreview, 1);

    // 显示间隔
    char info[24];
    snprintf(info, sizeof(info), "Every %d sec", repeatInterval);
    OLED_DrawString(0, 20, info, 1);

    OLED_DrawString(0, 28, "Any key to stop", 1);
    ...
    if (Keyboard.released) {
        ...
        case KEY_RIGHT: // Enter
            ...
            case MENU_SETTINGS:
                ...
                break;
            case MENU_REPEAT:
                uiState = UI_STATE_REPEAT_CONFIG;
                repeatSelection = REPEAT_SEL_CONTENT;
                repeatEditMode = false;
                memset(repeatEditBuffer, 0, sizeof(repeatEditBuffer));
                repeatEditLen = 0;
                break;
            case MENU_COUNT:
            default:
...
static void HandleSendInput(void) {
    ...
    if (Keyboard.released) {
        ...
        if (key == KEY_RIGHT && Keyboard.ctrlHeld) {
            CommitT9Char();
            if (inputLength < INPUT_BUFFER_SIZE - 1) {
                memmove(&inputBuffer[inputCursor + 1],
                        &inputBuffer[inputCursor], inputLength - inputCursor);
        ...
        case KEY_CTRL:
            // CapsLock toggled in keyboard driver
            displayNeedsUpdate = true;
            break;

        default:
            // T9 input keys (0-9, 1)
            if (key <= KEY_0 && key != KEY_UP && key != KEY_DOWN &&
                key != KEY_LEFT) {
                ...
                if (t9str && strlen(t9str) > 0) {
                    if (inputT9Active && Keyboard.lastT9Key == key) {
                        ...
                        inputT9Char = Keyboard_GetT9Char(key, Keyboard.t9Index);
                    } else {
                        // Different key or new input
                        CommitT9Char();
                        inputT9Char = Keyboard_GetT9Char(key, 0);
                        inputT9Active = true;
                    }
        ...
        if (settingEditMode) {
            ...
            default:
                ...
                if (key >= KEY_1 && key <= KEY_9) {
                    if (settingEditLen < sizeof(settingEditBuffer) - 1) {
                        settingEditBuffer[settingEditLen++] =
                            '0' + (key - KEY_1 + 1);
                        if (key == KEY_1)
                            settingEditBuffer[settingEditLen - 1] = '1';
                        else if (key == KEY_2)
                            settingEditBuffer[settingEditLen - 1] = '2';
                        else if (key == KEY_3)
                            settingEditBuffer[settingEditLen - 1] = '3';
                        else if (key == KEY_4)
                            settingEditBuffer[settingEditLen - 1] = '4';
                        else if (key == KEY_5)
                            settingEditBuffer[settingEditLen - 1] = '5';
                        else if (key == KEY_6)
                            settingEditBuffer[settingEditLen - 1] = '6';
                        else if (key == KEY_7)
                            settingEditBuffer[settingEditLen - 1] = '7';
                        else if (key == KEY_8)
                            settingEditBuffer[settingEditLen - 1] = '8';
                        else if (key == KEY_9)
                            settingEditBuffer[settingEditLen - 1] = '9';
                    }
                    displayNeedsUpdate = true;
                } else if (key == KEY_0) {
                    if (settingEditLen < sizeof(settingEditBuffer) - 1) {
            ...
            case KEY_UP:
                if (settingSelection > 0)
                    ...
                    settingSelection = SETTING_COUNT - 1;
                displayNeedsUpdate = true;
                break;

            case KEY_DOWN:
                if (settingSelection < SETTING_COUNT - 1)
                    settingSelection++;
                else
                    settingSelection = 0;
                displayNeedsUpdate = true;
...
static void HandleRepeatConfigInput(void) {
    if (Keyboard.released) {
        uint8_t key = Keyboard.releasedKey;
        ...
        case KEY_UP:
            if (!repeatEditMode) {
                if (repeatSelection > 0)
                    ...
                    repeatSelection = REPEAT_SEL_COUNT - 1;
                displayNeedsUpdate = true;
            }
            break;
        ...
        case KEY_DOWN:
            if (!repeatEditMode) {
                if (repeatSelection < REPEAT_SEL_COUNT - 1)
                    repeatSelection++;
                else
                    repeatSelection = 0;
                displayNeedsUpdate = true;
        ...
        case KEY_RIGHT: // Enter
            if (repeatSelection == REPEAT_SEL_CONTENT) {
                if (repeatEditMode) {
                    // 完成内容编辑，复制inputBuffer到repeatContent
                    memcpy(repeatContent, inputBuffer, inputLength);
                    repeatContentLen = inputLength;
                    repeatEditMode = false;
                    ClearInputBuffer();
                } else {
                    // 开始编辑内容
                    repeatEditMode = true;
                    // 将当前repeatContent复制到inputBuffer
                    memcpy(inputBuffer, repeatContent, repeatContentLen);
                    inputLength = repeatContentLen;
                    inputCursor = inputLength;
                }
                displayNeedsUpdate = true;
            } else if (repeatSelection == REPEAT_SEL_INTERVAL) {
                if (repeatEditMode) {
                    // 完成间隔编辑
                    int val = 0;
                    for (uint8_t i = 0; i < repeatEditLen; i++) {
                        val = val * 10 + (repeatEditBuffer[i] - '0');
                    }
                    if (val > 0 && val <= 999) {
                        repeatInterval = val;
                    }
            ...
            // 如果有内容，并且不在编辑模式，按Enter开始重复发送
            if (!repeatEditMode && repeatContentLen > 0) {
                uiState = UI_STATE_REPEAT_RUNNING;
                repeatLastSendTime = 0; // 立即发送第一次
                displayNeedsUpdate = true;
            }
            break;

        case KEY_SHIFT: // Back/Delete
            if (repeatEditMode) {
                if (repeatSelection == REPEAT_SEL_CONTENT) {
                    // 退格删除内容
                    if (inputCursor > 0) {
                    ...
                    if (repeatEditLen > 0) {
                        repeatEditLen--;
                        repeatEditBuffer[repeatEditLen] = '\0';
                    } else {
                        repeatEditMode = false;
                    }
                }
        ...
        default:
            ...
            if (repeatEditMode) {
                if (repeatSelection == REPEAT_SEL_INTERVAL) {
                    ...
                    if (key >= KEY_1 && key <= KEY_9) {
                        if (repeatEditLen < 3) {
                            repeatEditBuffer[repeatEditLen++] =
                                '1' + (key - KEY_1);
                        }
                        displayNeedsUpdate = true;
                    } else if (key == KEY_0) {
                        if (repeatEditLen < 3 &&
                            repeatEditLen > 0) { // 不允许以0开头
                            repeatEditBuffer[repeatEditLen++] = '0';
                        }
                        displayNeedsUpdate = true;
                    }
                } else if (repeatSelection == REPEAT_SEL_CONTENT) {
                    // T9输入内容
                    if (key <= KEY_0) {
                        const char *t9str = Keyboard_GetT9String(key);
                        if (t9str && strlen(t9str) > 0) {
                            if (inputT9Active && Keyboard.lastT9Key == key) {
                                ...
                                inputT9Char = Keyboard_GetT9Char(key, idx);
                            } else {
                                CommitT9Char();
                                inputT9Char = Keyboard_GetT9Char(key, 0);
                                inputT9Active = true;
    ...
    // T9超时处理
    if (repeatEditMode && repeatSelection == REPEAT_SEL_CONTENT) {
        if (inputT9Active && (GetTick() - inputT9LastTime > 1500)) {
            CommitT9Char();
            displayNeedsUpdate = true;
        }
...
static void HandleRepeatRunningInput(void) {
    // 任意按键停止重复发送
    if (Keyboard.released) {
        uiState = UI_STATE_REPEAT_CONFIG;
        displayNeedsUpdate = true;
...
void App_Task(void) {
    ...
    case UI_STATE_SETTINGS:
        HandleSettingsInput();
        break;
    case UI_STATE_REPEAT_CONFIG:
        HandleRepeatConfigInput();
        break;
    case UI_STATE_REPEAT_RUNNING:
        HandleRepeatRunningInput();
        // 检查是否需要发送
        {
            uint32_t currentTime = GetTick();
            uint32_t intervalMs = (uint32_t)repeatInterval * 1000;
            if (currentTime - repeatLastSendTime >= intervalMs) {
                // 发送消息
                E22_Send((uint8_t *)repeatContent, repeatContentLen);
                repeatLastSendTime = currentTime;
                displayNeedsUpdate = true;
            }
        }
        break;
...
void App_UpdateDisplay(void) {
    ...
    case UI_STATE_SETTINGS:
        ...
        break;
    case UI_STATE_REPEAT_CONFIG:
        DrawRepeatConfigScreen();
        break;
    case UI_STATE_REPEAT_RUNNING:
        DrawRepeatRunningScreen();
        break;
    }
}

void App_OnMessageReceived(uint8_t *data, uint8_t len, int8_t rssi,
                           int8_t snr) {
    // 清除接收状态
    App_IsReceiving = false;
...
void App_ExportHistory(void) {
    for (uint8_t i = 0; i < msgHistoryCount; i++) {
        uint8_t idx =
            (msgHistoryHead + MSG_HISTORY_SIZE - msgHistoryCount + i) %
            MSG_HISTORY_SIZE;
        Message_t *msg = &msgHistory[idx];
...
uint8_t App_GetHistoryCount(void) { return msgHistoryCount; }

Message_t *App_GetHistoryMessage(uint8_t index) {
    if (index >= msgHistoryCount)
        return NULL;
    uint8_t idx =
        (msgHistoryHead + MSG_HISTORY_SIZE - msgHistoryCount + index) %
        MSG_HISTORY_SIZE;
