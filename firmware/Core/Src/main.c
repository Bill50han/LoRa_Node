...
static void MX_GPIO_Init(void)
{
    ...
    // LoRa TXEN (PB10) - Output, initially low
    GPIO_InitStruct.Pin = LR_TXEN_PIN;
    GPIO_InitStruct.Mode = LL_GPIO_MODE_OUTPUT;
    GPIO_InitStruct.Speed = LL_GPIO_SPEED_FREQ_HIGH;
    GPIO_InitStruct.OutputType = LL_GPIO_OUTPUT_PUSHPULL;
    LL_GPIO_Init(LR_TXEN_PORT, &GPIO_InitStruct);
    LL_GPIO_ResetOutputPin(LR_TXEN_PORT, LR_TXEN_PIN);
    
    // LoRa RXEN (PB11) - Output, initially low
    GPIO_InitStruct.Pin = LR_RXEN_PIN;
    GPIO_InitStruct.Mode = LL_GPIO_MODE_OUTPUT;
    GPIO_InitStruct.Speed = LL_GPIO_SPEED_FREQ_HIGH;
    GPIO_InitStruct.OutputType = LL_GPIO_OUTPUT_PUSHPULL;
    LL_GPIO_Init(LR_RXEN_PORT, &GPIO_InitStruct);
    LL_GPIO_ResetOutputPin(LR_RXEN_PORT, LR_RXEN_PIN);
}
...
int main(void)
{
    ...
    while(1) {
        ...
        // Process UART commands (must be in main loop, not interrupt!)
